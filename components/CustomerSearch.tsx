...
export function CustomerSearch() {
  ...
  async function loadBookingsToReview(customerId: string) {
    ...
    const { data: existingReviews } = bookingIds.length
  ...
  async function loadProfessionals(nextLatitude = latitude, nextLongitude = longitude) {
    ...
    if (professionalIds.length) {
      const [{ data: servicesData }, { data: photosData }, { data: slotsData }, { data: reviewsData }] = await Promise.all([
  ...
  function servicesFor(professionalId: string) { return services.filter((service) => service.professional_id === professionalId).slice(0, 4); }
  ...
  async function requestBooking(professional: ProfessionalResult) {
    ...
    const selectedService = servicesFor(professional.professional_id)[0];
        ...
        <div className="professionalsGrid customerResults">
          {professionals.map((professional) => {
            ...
            return <article className="professionalCard customerProfessionalCard" key={professional.professional_id}>
              <div className="customerProfileTop">{professional.avatar_url ? <img className="profileAvatar" src={professional.avatar_url} alt={professional.full_name} /> : <span className="profileAvatarFallback">{initials || "ME"}</span>}<div><div className="cardTitleRow"><strong>{professional.full_name}</strong><span>Verificada</span></div><p>{professional.bio || "Atendimento profissional de manicure e pedicure em domicilio."}</p><div className="metaRow"><span>★ {Number(professional.average_rating || 0).toFixed(1)}</span><span>{Number(professional.distance_km || 0).toFixed(1)} km</span><span>{professional.review_count} avaliacoes</span></div></div></div>
